...
`VITE_DEFAULT_LANGUAGE` is kept for build compatibility, but this repository is now English-only. Set it to `en`.

Example:

```bash
# Build with English as the default language
docker build --build-arg VITE_DEFAULT_LANGUAGE=en -t bentopdf .
